...
1. **Via Nxpress CLI (`nxpress dev` / `nxp dev` / `nxpress export` / `nxpress routes`)**:
   ...
   - `nxpress routes`: Lists all generated API and page endpoints (`endpoint`, `methods`, `file`). Ideal for debugging unexpected routing issues, identifying missing routes, or catching unwanted generated endpoints.
